...
public class SplineElevator : Conveyance
{
    ...
    private void OnDrawGizmos()
    {
        ...
        if (Array.Exists(ControlPoints, go => go == null)) return;

        SplineInterpolator spline = SetupSpline(ControlPoints);
    ...
    public override void SetDestination()
    {
        ...
        _mSplineInterp = SetupSpline(ControlPoints); //+++
        //int hermite = -1; //+++
    ...
    private void Update()
    {
        ...
        Vector3 NextPosition = new Vector3 (_buttonPressed[0].x-5.35f, _buttonPressed[0].y, _buttonPressed[0].z);
        if (NextPosition.y > Car.transform.position.y)
        {
        ...
        if (Mathf.Abs(CarPosition.y - NextPosition.y) > 0.01f) return;
